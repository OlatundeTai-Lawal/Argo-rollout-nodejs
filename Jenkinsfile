pipeline {
    ...
    environment {
    DOCKERHUB_CREDENTIALS = credentials('ola-dockerhub-token')
    APP_NAME = "tundeficky/blue-green-rollout"
    }
    stages { 
        stage('SCM Checkout') {
            steps{
           git branch: 'main', url: 'https://github.com/OlatundeTai-Lawal/Argo-rollout-nodejs.git'
            }
        }
        // run sonarqube test
        stage('Run Sonarqube') {
